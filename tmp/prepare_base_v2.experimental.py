...
from reparos.base_v2 import BaseV2Config, prepare_base_v2


def main() -> None:
    parser = argparse.ArgumentParser(description="Audit or materialize deterministic ReparoS Base V2 data")
    parser.add_argument("--source", required=True, help="prepared-v4-leakfree root")
    parser.add_argument("--output", required=True)
    parser.add_argument("--profiles", default="8,16,32", help="comma-separated subset of 8,16,32")
    parser.add_argument("--seed", type=int, default=2026)
    parser.add_argument("--max-groups-per-split", type=int)
    parser.add_argument("--sample-per-family", type=int, default=20)
    parser.add_argument("--materialize", action="store_true", help="write src/tgt/meta files; default is dry-run")
