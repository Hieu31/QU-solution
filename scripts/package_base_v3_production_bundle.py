...
def main():
    ...
    out_zip = dist_dir / "reparos-base-v3-production-data.zip"

    if out_zip.exists():
        out_zip.unlink()

    print("==================================================================")
    print("      PACKAGING REPAROS BASE V3 PRODUCTION DATA BUNDLE (4M)       ")
    print("      Recipe: Run C Winning Configuration (Zero Leakage Certified) ")
    print("==================================================================")
    ...
    with zipfile.ZipFile(out_zip, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        # 3. Add Production Dataset (4M pairs)
        print("\n[3/4] Adding Production Dataset (data/base_v3_production/)...")
        prod_dir = repo_root / "data/base_v3_production"
        for root, _, files in os.walk(prod_dir):
            for file in files:
                # Do not package large intermediate checkpoints or temp files
                if file.endswith((".pt", ".tmp", ".log")):
                    continue
                full_path = Path(root) / file
                rel_path = full_path.relative_to(repo_root)
                print(f"  + {rel_path} ({full_path.stat().st_size / (1024*1024):.1f} MB)...", flush=True)
