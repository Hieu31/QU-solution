"""
Render self-contained Kaggle notebook to train and benchmark all 6 scaling arms (A through F)
...
def generate_notebook():
    ...
    cells.append({
        ...
        "source": [
            ...
            "\n",
            "This notebook systematically trains and benchmarks 6 Transformer architectures on the **100% pure clean Pilot Dataset (300,000 pairs)** to answer:\n",
            "1. **Does depth or width significantly improve sequence repair and address canonicalization?**\n",
            "2. **Does the gain primarily come from a deeper Encoder (Arm D) while keeping a fast Decoder?**\n",
            "3. **Is Knowledge Distillation justified?** (e.g. Teacher = Arm C/F, Student = Arm A/D).\n",
            "\n",
    ...
    cells.append({
        ...
        "source": [
            ...
            "REPORT_EVERY = 1_000\n",
            "\n",
            "# Select which arms to run (Default: all 6 arms)\n",
    ...
    cells.append({
        ...
        "source": [
            ...
            "    import ctranslate2\n",
            "    print(\"Dependencies (OpenNMT-py, sentencepiece, ctranslate2) ready.\")\n",
            "except ImportError:\n",
    ...
    # 5. Training Loop for the 6 Arms
    cells.append({
        "cell_type": "code",
        "execution_count": None,
        "metadata": {},
        ...
        "source": [
            ...
            "    \n",
            "    # Load config and override paths to match Kaggle environment\n",
            ...
            "        \n",
            "    print(f\"\\n{'='*70}\")\n",
            "    print(f\"STARTING TRAINING: {arm.upper()} (Enc: {cfg['enc_layers']}, Dec: {cfg['dec_layers']}, Dim: {cfg['hidden_size']})\")\n",
            "    print(f\"{'='*70}\")\n",
            "    \n",
            ...
            "        print(f\"ERROR in {arm}! Returncode: {proc.returncode}\")\n",
            "        raise RuntimeError(f\"Training failed for {arm}\")\n",
            "        \n",
            "    print(f\"Completed {arm} in {t_dur/60:.1f} minutes!\")\n",
            "    training_results[arm] = {\"duration_sec\": t_dur}\n"
        ]
    })
    
    # 6. CTranslate2 Conversion
    cells.append({
        "cell_type": "code",
        "execution_count": None,
        "metadata": {},
        ...
        "source": [
            "ct2_dirs = {}\n",
            "print(\"\\nConverting checkpoints to CTranslate2 (INT8 / FP16)...\")\n",
            "\n",
            "for arm in ACTIVE_ARMS:\n",
            "    arm_ckpt_dir = checkpoints_root / arm\n",
            "    ckpts = sorted(list(arm_ckpt_dir.glob('model_step_*.pt')))\n",
            "    assert ckpts, f\"No checkpoint found for {arm}!\"\n",
            "    latest_ckpt = ckpts[-1]\n",
            "    \n",
            ...
            "        \n",
            "    print(f\"Converting {latest_ckpt.name} for {arm}...\")\n",
            "    cmd = [\n",
            ...
            "    ct2_dirs[arm] = arm_ct2_dir\n",
            "    print(f\"  Converted successfully to {arm_ct2_dir}\")\n"
        ]
    })
    
    # 7. Benchmark Evaluation (6 Frozen Suites + Diagnostics)
    cells.append({
        "cell_type": "code",
        "execution_count": None,
        "metadata": {},
        ...
        "source": [
            ...
            "            out_text = sp.decode_pieces(trans[0]['tokens'])\n",
            ...
            "    return acc, lat_ms\n",
            "\n",
            "# Run evaluation across all arms\n",
            "eval_matrix = {}\n",
            "suites = {\n",
            ...
            "}\n",
            "\n",
            "print(\"\\nBenchmarking all arms on Frozen Test Suites...\")\n",
            "for arm, ct2_dir in ct2_dirs.items():\n",
            "    print(f\"Evaluating {arm}...\")\n",
            "    translator = ctranslate2.Translator(str(ct2_dir), device='cuda' if ctranslate2.get_cuda_device_count() > 0 else 'cpu')\n",
            "    eval_matrix[arm] = {}\n",
            "    \n",
            "    for s_name, (s_src, s_tgt) in suites.items():\n",
            "        acc, lat = eval_suite(translator, s_src, s_tgt)\n",
            "        eval_matrix[arm][s_name] = acc\n",
            "        eval_matrix[arm][f\"{s_name}_lat\"] = lat\n",
            "        print(f\"  {s_name}: {acc:.2f}% (lat: {lat:.2f}ms)\")\n",
            "        \n",
            "    # Overall mean accuracy\n",
            "    mean_acc = sum(eval_matrix[arm][s] for s in suites) / len(suites)\n",
            "    mean_lat = sum(eval_matrix[arm][f\"{s}_lat\"] for s in suites) / len(suites)\n",
            "    eval_matrix[arm][\"mean_acc\"] = mean_acc\n",
            "    eval_matrix[arm][\"mean_lat\"] = mean_lat\n",
            "    print(f\"  >> OVERALL MEAN: {mean_acc:.2f}% | Latency: {mean_lat:.2f}ms\")\n"
        ]
    })
    
    # 8. Markdown Summary Table and Distillation Decision
    cells.append({
        "cell_type": "code",
        "execution_count": None,
        "metadata": {},
        ...
        "source": [
            "print(\"\\n\" + \"=\"*90)\n",
            "print(\"### FINAL EMPIRICAL SCALING LAWS COMPARISON MATRIX\")\n",
            "print(\"=\"*90)\n",
            "\n",
            "header = \"| Arm | Arch | Train Time | P50 Latency (ms) | Mean Accuracy | Plasticity (Telex) | Retention (OSM) | User Centric |\"\n",
            "divider = \"|---|---|---|---|---|---|---|---|\"\n",
            "print(header)\n",
            "print(divider)\n",
            "\n",
            "for arm in ACTIVE_ARMS:\n",
            "    m = eval_matrix[arm]\n",
            "    t_min = training_results[arm]['duration_sec'] / 60\n",
            ...
            "best_arm = max(ACTIVE_ARMS, key=lambda a: eval_matrix[a]['mean_acc'])\n",
            "best_acc = eval_matrix[best_arm]['mean_acc']\n",
            "best_lat = eval_matrix[best_arm]['mean_lat']\n",
            "gain = best_acc - baseline_acc\n",
            "\n",
            "print(\"\\n\" + \"=\"*90)\n",
            "print(f\"SCALING VERDICT: Best Arm = {best_arm} ({best_acc:.2f}%, Delta vs Baseline: +{gain:.2f}%)\")\n",
            "\n",
            "if gain >= 5.0 and best_lat > 5.5:\n",
            "    print(\"==> DECISION: ADOPT KNOWLEDGE DISTILLATION!\")\n",
            "    print(f\"    Deep model {best_arm} provides significant accuracy gain (+{gain:.2f}%), but exceeds 5.5ms latency SLA.\")\n",
            "    print(f\"    RECOMMENDATION: Train {best_arm} as Teacher on full 4M data, then distill into Arm A or Arm D.\")\n",
            "elif eval_matrix['arm_d_4e1d_d128']['mean_acc'] >= (best_acc - 1.0) and eval_matrix['arm_d_4e1d_d128']['mean_lat'] <= 5.5:\n",
            "    print(\"==> DECISION: ADOPT ASYMMETRIC ARM D (4E1D d128)!\")\n",
            "    print(\"    Arm D captures virtually all the depth gain while maintaining < 5.5ms latency SLA.\")\n",
            "    print(\"    RECOMMENDATION: Scale Arm D directly for production without complex distillation!\")\n",
            "else:\n",
            "    print(\"==> DECISION: NO DISTILLATION NEEDED!\")\n",
            "    print(\"    Scaling depth/width provides marginal gain (< 2%). The 2E1D baseline capacity is sufficient.\")\n",
            "    print(\"    RECOMMENDATION: Keep Arm A (2E1D), prioritize data diversity and coverage over model size.\")\n",
            "print(\"=\"*90)\n"
        ]
    })
    ...
    out_path = Path(__file__).resolve().parent.parent / "notebook" / "benchmark_scaling_arms_pilot_kaggle.ipynb"
